...
_verbosity = Timba.utils.verbosity(name="lsmimport");
...
def importASCII_DMS (filename,format=DefaultDMSFormat,freq0=None):
  ...
  for linenum,line in enumerate(file(filename)):
    try:
      ...
      srclist.append(ModelClasses.Source(name,pos,flux,shape=shape,spectrum=spectrum,**dict([(tag,True) for tag in tags])));
    except:
...
def importNEWSTAR (filename,import_src=True,import_cc=True,**kw):
  ...
  for ii in range(0,nsources):
    ...
    src = ModelClasses.Source(uniqname,pos,flux,shape=shape,spectrum=spectrum,**tags);
    srclist.append(src);
  dprintf(2,"imported %d sources from file %s\n",len(srclist),filename);
