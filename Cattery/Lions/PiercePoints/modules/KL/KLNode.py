...
class KLNode (pynode.PyNode):
    ...
    def get_result (self,request,*children):
        ...
        res1=children[1];
        vs1=res1.vellsets; #num_pierce_points each containing vector of length 2 or 3 (x,y(,z))
        dims = res1.dims;
        vector_size=dims[1];
        num_pp=dims[0];
        x=[];
        ...
        #Then the parameters
        res0=children[0];
        vs0=res0.vellsets; # should be one per parameter
        num_parms=len(vs0);
        val0=[0,]*num_parms;
        par=[0,]*num_parms;
        solvable=False;
        ...
        for pn in range(num_parms):
            ...
            par[pn]=val0[pn][0]; #use first value (in case of constant parms)
