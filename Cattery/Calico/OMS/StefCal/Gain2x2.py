...
##      self.gain = dict([ (p,(default,self._zero,self._zero,default)) for p in self._antennas ]);
  ...
  def _get_matrix (self,p,q,data):
    if (p,q) in data:
      return list(map(self._tile_complex_data,data[p,q]));
    elif (q,p) in data:
      return matrix_conj(list(map(self._tile_complex_data,data[q,p])));
    else:
      return None;

  def _get_conj_matrix (self,p,q,data):
    if (p,q) in data:
      return matrix_conj(list(map(self._tile_complex_data,data[p,q])));
    elif (q,p) in data:
      return list(map(self._tile_complex_data,data[q,p]));
    else:
      return None;
